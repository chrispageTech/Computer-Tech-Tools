...
def click():
    ...
    subprocess.run(['I:\\Python\\MBSetup.exe'], shell=True)
    button.config(bg='white')

def click1():
    print("Launching SIW!")
    subprocess.run(['I:\\Python\\siw\\siw.exe'], shell=True)
    button1.config(bg='white')

def click2():
    print("Launching CystalDisk!")
    subprocess.run(['I:\\Python\\CrystalDiskInfo7_1_1\\DiskInfo64.exe'], shell=True)
    button2.config(bg='white')
    
def click3():
    print("Launching SmartDefrag!")
    subprocess.run(['I:\\Python\\SmartDefragPortable\\SmartDefragPortable.exe'], shell=True)
    button3.config(bg='white')

def click4():
    print("Launching ADWCleaner!")
    subprocess.run(['I:\\Python\\adwcleaner.exe'], shell=True)
    button4.config(bg='white')

def click5():
    print("Launching HerdProtect!")
    subprocess.run(['I:\\Python\\herdProtectScan_Portable.exe'], shell=True)
    button5.config(bg='white')

def click6():
    print("Launching HitmanPro!")
    subprocess.run(['I:\\Python\\HitmanPro_x64.exe'], shell=True)
    button6.config(bg='white')
    
def click7():
    print("Launching RogueKiller!")
    subprocess.run(['I:\\Python\\RogueKiller_portable64.exe'], shell=True)  
    button7.config(bg='white')
    
def click8():
    print("Launching TDSSKiller!")
    subprocess.run(['I:\\Python\\TDSSKiller.exe'], shell=True)
    button8.config(bg='white')

def click9():
    print("Launching TFC!")
    subprocess.run(['I:\\Python\\TFC.exe'], shell=True)
